...
@section('content')
            ...
            async function fetchDoctors(specialiteId, page = 1) {
                ...
                try {
                    ...
                    paginatedData.forEach(medecin => {
                        const imageUrl = medecin.photo ? `${baseUrl}${medecin.photo}` :
    ...
    dateHeureDebut.textContent = `Date : ${creneau.jour} |  ${creneau.heure_debut} |  ${creneau.heure_fin}`;
    ...
    disponibiliteRadio.value = creneau.jour; // Utilisation du jour de disponibilité comme valeur
    disponibiliteRadio.id = `disponibilite-${creneau.jour}`;
    disponibiliteRadio.classList.add('form-check-input', 'me-2');

    // Label pour le bouton radio
    const label = document.createElement('label');
    label.htmlFor = `disponibilite-${creneau.jour}`;
