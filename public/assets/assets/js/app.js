$(document).ready(function ($) {
    ...
    // Vérifier si une table avec la classe "datatable" existe
    if ($(".datatable").length > 0) {
        // Initialiser DataTables avec les options spécifiées
        $(".datatable").DataTable({
            bFilter: false,
            lengthChange: false, // Cette ligne masque l'option "Show" de la pagination
            dom: "Bfrtip",
            buttons: [
    ...
    // Déplacer les boutons vers la div "download-grp"
    $(".download-grp").html($(".dt-buttons"));

    // Réinitialiser les événements de clic pour les boutons d'exportation

    $("#export-excel").on("click", function () {
        $(".datatable").DataTable().button("2").trigger();
    });

    $("#export-pdf").on("click", function () {
        $(".datatable").DataTable().button("3").trigger();
    });

    $("#export-print").on("click", function () {
        $(".datatable").DataTable().button("4").trigger();
    });
    ...
    function filterTable() {
        ...
        for (i = 0; i < tr.length; i++) {
            ...
            var matchFound = false; // Ajout d'une variable pour suivre si une correspondance a été trouvée
            for (j = 0; j < td.length; j++) {
                if (td[j]) {
                    txtValue = td[j].textContent || td[j].innerText;
                    if (txtValue.toUpperCase().indexOf(filter) > -1) {
                        tr[i].style.display = "";
                        matchFound = true; // Mettre à jour la variable
                        break;
                    }
                }
            }
            // Vérifier si une correspondance a été trouvée et masquer la ligne si aucune correspondance n'a été trouvée
            if (!matchFound) {
    ...
    // Écouter les événements de saisie pour déclencher le filtrage automatique
    document
    ...
    if ($(".dash-count .counter-up").length > 0) {
        $(".dash-count .counter-up").counterUp({ delay: 15, time: 1500 });
    }
    if ($(".summernote").length > 0) {
        $(".summernote").summernote({
            height: 200,
